...
def filtr(inpt, isFile=False):
    return (
        "".join(i for i in inpt if i not in r'<>:"/\|?*')
        if isFile
        else "".join(i.lower() for i in inpt if i.lower() in letters)
    )
...
def normalGet(artist="", title="", _type=0):
    ...
    return (
        "https://www.azlyrics.com/{}/{}.html".format(art[0], art)
        if type
        else "https://www.azlyrics.com/lyrics/{}/{}.html".format(art, tit)
    )
...
def googleGet(srch_eng, acc, get_func, artist="", title="", _type=0, proxies={}):
    ...
    google_page = get_func(
        "{}{}+site%3Aazlyrics.com".format(
            search_engines[slctd_srch_engn], encoded_data
        ),
        proxies,
    )
